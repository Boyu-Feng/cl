...
2026-09-25 完整结果：**2010/2010** 方法／任务／种子单元，134 题 × 3 种子 = 402 个共同配对，无基础设施运行错误。下表包含每条类型链的首题；排除首题后另外报告 384 个配对。

| 方法 | 首次成功 / 402 | 三次内成功 / 402 | 三次内成功率 |
|---|---:|---:|---:|
| 无经验重试 | 78 | 101 | 25.12% |
| 未训练生成器 | 69 | 124 | 30.85% |
| 旧 Delta | 74 | 145 | 36.07% |
| Reflexion | 78 | 200 | 49.75% |
| ExpeL | 130 | 141 | 35.07% |

旧 Delta 相对无经验重试提升 10.95 个百分点，任务聚类 bootstrap 95% 区间为 +4.73 至 +17.41 个百分点；三个种子分别提升 +11.94、+15.67、+5.22 个百分点。相对未训练生成器提升 5.22 个百分点，三个种子分别为 +2.99、+8.96、+3.73。相对 ExpeL 仅多 4/402 次成功，不足以认定稳定领先；Reflexion 的三次内成功率明显更高，ExpeL 的首次成功率最高。旧 Delta 首次成功率为 18.41%，无经验为 19.40%，没有首次尝试提升。

排除每个类型／种子链首题后，三次内成功依次为 **95、116、137、191、133 / 384**。另 20 道此前未评估题 × 3 种子的三次内成功依次为 **6、20、18、30、14 / 60**，该小子集类型不均衡。完整结果来自 `ttcl/results/alfworld_comparison/20260924_parserfix/{summary.json,REPORT.md}`。
...
2026-09-25 两轮均结束，各完成 792/792 条开发记录、无缺失开发分数，**均未通过双基准开发门槛，没有启动后缀确认测试**。第一轮保留 12 对训练偏好，各组 12 次优化；第二轮累计保留 26 对（ALF 20、数据库 3、扑克 3），各组 30 次优化。频谱没有确认训练对，队列确认对因超出旧训练上下文预算被排除。

下表均排除序列首题；ALF 每组 84 个任务／种子配对，扑克每组 14 个。

| 方法 | 第 1 轮 ALF 成功 | 第 2 轮 ALF 成功 | 第 1 轮扑克均值 | 第 2 轮扑克均值 |
|---|---:|---:|---:|---:|
| 无经验 | 23/84 | 24/84 | 4.3571 | 5.4286 |
| 未训练生成器 | 21/84 | 22/84 | 1.4643 | 8.8929 |
| 旧 Delta | 21/84 | 15/84 | 1.6429 | 4.7143 |
| 筛选经验 SFT | 24/84 | 19/84 | 2.7143 | 9.2500 |
| 随机经验 SFT | 24/84 | 18/84 | 0.6786 | 2.0000 |
| DPO | 17/84 | 22/84 | 0.2143 | 2.2143 |

两轮频谱全部持平 0.21772（各组 10 条），数据库全部 0（各组 2 条）。队列第一轮／第二轮：无经验 −0.01377／−0.01944，未训练 +0.00135／+0.00024，旧 Delta +0.00063／−0.00257，筛选 SFT −0.02231／−0.02529，随机 SFT +0.00231／−0.02462，DPO −0.01204／+0.00053（各组仅 2 条，来自排除首题后的一道独立题，不能把退化的聚类区间当高置信结论）。

第二轮筛选 SFT 在扑克有正向点估计，但在 ALF 低于无经验，且队列退步；DPO 在 ALF 优于同轮旧 Delta但未超过无经验，扑克退步。两轮基线自身也有采样／推理运行波动，应在同轮内配对，不能把两轮相减视为训练因果效应。结果位于 `results/experience_lab/20260924_r01/round_00{1,2}/development/`。
...
只读诊断确认同一 token 序列上，BF16 SDPA 默认实现、memory-efficient 实现、数学实现和 FP32 的概率存在差别；初始 reader 的零 LoRA 与关闭 LoRA 完全一致。切换实现可修复个别样本，但不能同时消除所有偏差，不能直接提高阈值后声称严格同策略。
...
python -m ttcl.experience_coop.run recover \
  --origin "$PWD/results/experience_coop/20260924_ppo8" \
  --root "$PWD/results/experience_coop/20260925_ppo8_corrected" --gpu 3 --port 18287
python -m ttcl.experience_coop.run launch \
