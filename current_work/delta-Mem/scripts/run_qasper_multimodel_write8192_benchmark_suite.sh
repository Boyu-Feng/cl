...
SUITE_ROOT="${SUITE_ROOT:-${ROOT_DIR}/model/qasper_multimodel_write8192_benchmark_suite_consistent70}"
...
MEMORY_AGENT_BENCH_ROOT="${MEMORY_AGENT_BENCH_ROOT:-${ROOT_DIR}/external/MemoryAgentBench}"
...
SSW_ADAPTER_DIR="${SSW_ADAPTER_DIR:-${ROOT_DIR}/model/qwen3_4b_ssw_output/trainer/checkpoint-70}"
TSW_ADAPTER_DIR="${TSW_ADAPTER_DIR:-${ROOT_DIR}/model/tsw_output/trainer/checkpoint-70}"
MSW_ADAPTER_DIR="${MSW_ADAPTER_DIR:-${ROOT_DIR}/model/msw_output/trainer/checkpoint-70}"

if [[ -n "${CUDA_VISIBLE_DEVICES:-}" ]]; then
  DEFAULT_NPROC_PER_NODE="$(awk -F, '{print NF}' <<< "${CUDA_VISIBLE_DEVICES}")"
else
  DEFAULT_NPROC_PER_NODE="$(nvidia-smi --list-gpus 2>/dev/null | wc -l)"
  [[ "${DEFAULT_NPROC_PER_NODE}" -gt 0 ]] || DEFAULT_NPROC_PER_NODE=1
fi
