...
# Number of DDP workers.  CUDA_VISIBLE_DEVICES is commonly a comma-separated
# list (e.g. 0,1,2,3,4), so derive its length rather than passing the string.
...
COMMON_TRAIN_ARGS=(
  ...
  --max-length 512
  # 8192-token writes keep a very large hidden-state/activation footprint on
  # every DDP rank.  Keep this conservative by default; override with
  # MAX_WRITE_LENGTH when more memory is available.
  --max-write-length "${MAX_WRITE_LENGTH:-2048}"
