...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dataset-name",
        default="huaXiaKyrie/delta-mem-qasper-data",
    )
...
def load_examples(args: argparse.Namespace) -> Dataset:
    ...
    if args.dataset_name is not None:
        try:
            ...
            # Legacy caches encode list-of-struct as `_type: List`, while
            # current datasets no longer exposes that feature class. Patch
            # only the deserializer so it produces the native list syntax.
            _original_generate = _features.generate_from_dict
            def _generate_legacy_compatible(obj):
                if isinstance(obj, dict) and obj.get("_type") == "List":
                    return [_generate_legacy_compatible(obj["feature"])]
                return _original_generate(obj)
            _features.generate_from_dict = _generate_legacy_compatible
            loaded = load_dataset(args.dataset_name, cache_dir=cache_dir)
