...
def load_delta_mem_chat_model(
    *,
    ...
    if device_map not in (None, "single", "layerwise"):
        raise ValueError("device_map must be 'single' or 'layerwise'")
    placement = None
    if device_map == "layerwise":
        from transformers import AutoConfig

        from deltamem.runtime.device_mapping import layerwise_device_map

        model_config = AutoConfig.from_pretrained(model_path, local_files_only=True)
        if model_config.model_type not in ("qwen3", "smollm3"):
            raise ValueError("layerwise inference supports Qwen3 and SmolLM3 backbones")
        placement = layerwise_device_map(
            model_config.num_hidden_layers, torch.cuda.device_count()
        )
    tokenizer = AutoTokenizer.from_pretrained(model_path, local_files_only=True)
    model = AutoModelForCausalLM.from_pretrained(
        model_path,
        dtype=get_dtype(dtype),
        # Attach Delta-Mem BEFORE dispatch so hooks see the final module tree.
        device_map={"": "cpu" if placement is not None else device},
    ...
    if placement is not None:
        ...
        model = dispatch_model(
            ...
            main_device=0,
            # KV caches must remain on their owning decoder block's device.
