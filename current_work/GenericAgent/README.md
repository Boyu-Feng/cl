...
### Local Qwen3 on this server (no API key)

The local `mykey.py` selects `LocalQwenSession`, which loads the Qwen3 weights
directly with Transformers. Override the checkpoint/device with `GA_MODEL` and
`GA_DEVICE`. Local inference dependencies are available as the `[local]` extra.
The existing agent loop, tools, checkpoints and long-term memory workflow remain
in use. `GA_INLINE_PROMPT=1` optionally passes long requests inline instead of
replacing them with a file path.

```bash
# From current_work/GenericAgent, using the server's prepared environment:
GA_INLINE_PROMPT=1 /home/fengboyu/miniconda3/envs/seal_env/bin/python agentmain.py
```

For isolated CLBench tests, use [the TTCL launcher](../../ttcl/generic_agent/README.md).
