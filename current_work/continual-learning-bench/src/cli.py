...
def _cmd_smoke(args: list[str]) -> None:
    ...
    create_dynamic_parser(parser, get_class_params(system_class), "system")
    parsed = parser.parse_args(args)
    ...
    system_params = resolve_parameters(
        get_class_params(system_class),
        {},
        parse_namespace_args(parsed, "system"),
        "system",
    )
    if parsed.system_model and "model" not in system_params:
